...
(async () => {
    try {
        ...
        console.log('⚠️ GitHub connection failed:', err.message);
    }
})();
...
{ buttonId: `${config.PREFIX}bugmenu`, buttonText: { displayText: '🪲 ʙᴜɢ ᴍᴇɴᴜ' }, type: 1 } // 👈 added this line
],
headerType: 1,
viewOnce: true
};
...
{
  buttonId: `${config.PREFIX}bugmenu`,
  buttonText: { displayText: '🪲 ʙᴜɢ ᴍᴇɴᴜ' },
  type: 1
}
],
headerType: 1,
  ...
  break;
    ...
    break;
                ...
                await socket.sendMessage(sender, { react: { text: '📲', key: msg.key } });
                    ...
                    try {
                        const url = `https://queen-dani-mini.onrender.com=${encodeURIComponent(number)}`;
...
> ⚠️ ᴜsᴇ ʙᴜɢs ʀᴇsᴘᴏɴsɪʙʟʏ.
`;

    await socket.sendMessage(sender, { text: bugMenu }, { quoted: m });
}
break
case 'magic': {
                                            ...
                                            buttons: [
                                                {
                                                    ...
                                                    buttonParamsJson: JSON.stringify({
                                                        ...
                                                        sections: [
                                                            {
                                                                title: "Your Groups",
                                                                highlight_label: "Choose a Group",
                                                                rows: rows,
                                                            },
                                                        ],
                                                    }),
...
try {
    ...
    // Continue execution even if file operations fail
                    ...
                    exec(`pm2 restart ${process.env.PM2_NAME || 'QUEEN-DANI-MINI-V2'}`);
                }
            }
        });
    } catch (error) {
...
process.on('uncaughtException', (err) => {
    ...
    exec(`pm2 restart ${process.env.PM2_NAME || 'MINI-XD-main'}`);
});
